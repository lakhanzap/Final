...
define('CHMPURL', WP_PLUGIN_URL . '/' . str_replace(basename( __FILE__),"",plugin_basename(__FILE__)) );
 ...
 function club_plugin_add_option_page()
{
  ...
  add_menu_page   ('Clubhouse Manager Plugin Options', 'Clubhouse Manager', 8, 'club-option-page',  'club_plugin_create_option_page', CHMPURL . 'images/clubhouseManager_16x16.gif');
...
//  add_submenu_page( 'club-option-page', 'Import',                'Import', 5, 'club-import', 'club_plugin_create_import_page');
  ...
  add_submenu_page( 'club-option-page', 'Statastics and Result',                 '',  5, 'club-statsticsgame',  'club_plugin_create_statsticsgame_page');
...
function  club_print_scripts()
{
 ...
 wp_enqueue_script( 'club_admin_script', plugin_dir_url( __FILE__ ) . 'includes/js/club_admin_scripts.js', array('jquery', 'json2'), false, false);
  wp_enqueue_script( 'club_admin_validation_script', plugin_dir_url( __FILE__ ) . 'includes/js/jquery.validate.min.js', array('jquery', 'json2'), false, false);
 wp_enqueue_script( 'club_admin_datepicker_script', plugin_dir_url( __FILE__ ) . 'includes/js/jquery-ui.js', array('jquery', 'json2'), false, false);
      ...
      $tab = "";
          ...
          if($_GET["tab"] == "1" || $_GET["tab"] =="" ){
            ...
            $tab = 'jQuery(".tab_content:first").show();';

          }else if($_GET["tab"] == "2"){

            $tab = 'jQuery(".tab_content:nth-child(2)").show();';

          }else if($_GET["tab"] == "3"){

            $tab = 'jQuery(".tab_content:nth-child(3)").show();';

          }else if($_GET["tab"] == "4"){

            $tab = 'jQuery(".tab_content:nth-child(4)").show();';
   ...
   jQuery("ul.tabs li:first").addClass("active");
        ...
        jQuery("ul.tabs li").click(function() {
          ...
          jQuery(activeTab).fadeIn(10); //Fade in the active content
          return false;
        });
      });
      </script>';
...
function show_tinyMCE() {
    ...
    wp_enqueue_style( 'club_datepicker_styles', CHMPURL . 'css/jquery-ui.css');
}
...
function  club_print_styles()
{
  if ( is_admin() )
  {
    wp_enqueue_style( 'jPicker_styles', CHMPURL . 'css/jPicker-1.1.5.min.css');
//  wp_register_style('club_admin_styles', CHMPURL . 'css/club-admin-plugin.css');
    wp_enqueue_style( 'club_admin_styles', CHMPURL . 'css/club-admin-plugin.css');
    
  }
  else
  {
    wp_register_style('table_sorter_styles', CHMPURL . 'css/blue/style.css');
    wp_register_style('club_frontend_styles', CHMPURL . 'css/club-frontend-plugin.php');
...
function club_plugin_create_players_page()
{
  ...
  $edit_player = false;
    foreach( $_POST as $key => $value )
    {
      ...
      if ( !(strpos( $key, 'club_edit_player_') === false) )
      {
        ...
        $id = $_POST['club_players_assign_select'][0];
  ...
  if ( $_POST['club_save_player_btn'] )
  {
    ...
    if ( !(strpos( $key, 'club_update_player_') === false) )
    {
     // echo "asd";die;
        ...
        $pos1= strpos( $key, '_btn', $pos);
        $player_id = (int)substr( $key, ($pos + 19), ($pos1 - ($pos + 19)) );
        $season  = club_get_option('club_players_season');
      //  echo 'season'.$season;
  ...
  if ( $_POST['club_assign_players_team_btn'] )
  {
    ...
    else
    {
     ...
     $jersey_num = $_POST['club_player_edit_jerseynum'];
      club_assign_players_team($team, $season, $players_selected,$jersey_num); // print_r($_POST);
 //       exit;
      if(mysql_error())
      {

          echo '<div id="message" class="error fade">';
      echo '<strong>Players not added to team !!!</strong></div>';
      }
      else
      { 
          echo '<div id="message" class="updated fade">';
...
function  club_plugin_create_Statsticsgame_page()
{
 // echo "<pre>";print_r($_POST);die;
  ...
  club_plugin_print_Statsticsgame_page($_POST);

  return;
}
////////////////////////////////////////////////////////////////////////////////
/*function  club_plugin_create_tournaments_page()
{
  //  check if one edit button is pressed
  $edit_tournament = false;
  foreach( $_POST as $key => $value )
  ...
  club_plugin_print_tournaments_page($edit_tournament);
...
//echo "<pre>";print_r($_POST);die;
    foreach( $_POST as $key => $value )
    {
      if ( !(strpos( $key, 'club_delete_field_') === false) )
      {
        ...
        if ( !(strpos( $key, '_btn') === false) )
        {
          ...
          club_delete_location( $fields[$id]['field_id'] );
          $field_deleted = true;
          break;
        }
      }
  ...
  if ( $_POST['club_save_location_btn'] )
  {
    ...
    else
    {
      //echo "<pre>";
      //  location update
      if(isset($_POST['club_field_edit_fieldname']))
      {
         //echo "<pre>";print_r($_POST);die;
        $field_id = $_POST['club_delete_field_id'];
        $field_edit_id = $_POST['field_id']+1;
        $fieldname  = $_POST['club_field_edit_fieldname'];
        $directions = $_POST['club_field_edit_directions'];

        $ret        = club_update_location($fieldname, $directions,$field_edit_id);
      }
...
//   exit;
     ...
     //    exit; 
  {
    if ( !(strpos( $key, 'club_edit_game_') === false) )
    {
      ...
      $pos1= strpos( $key, '_btn');
      $id = (int)substr( $key, ($pos + 15), ($pos1 - ($pos + 15)) );
      club_update_option('club_game_edit_id', $id);
      $edit_game = true;
