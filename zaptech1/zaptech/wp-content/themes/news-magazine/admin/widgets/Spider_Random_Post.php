<?php
/// random post widget
$news_magazine_spider_random_post_id=0;

if(!class_exists('news_magazine_spider_random_post')){
	class news_magazine_spider_random_post extends WP_Widget {

	// Constructor //

		function news_magazine_spider_random_post() {
			$widget_ops = array( 'classname' => 'news_magazine_spider_random_post', 'description' => 'Spider Random Post allows you to show posts in a random order in a sidebar.' ); // Widget Settings
			$control_ops = array( 'id_base' => 'news_magazine_spider_random_post' ); // Widget Control Settings
			$this->WP_Widget( 'news_magazine_spider_random_post', 'Spider Random Post', $widget_ops, $control_ops ); // Create the widget
		}
	...
	// Extract Args //
		...
		function widget($args, $instance) {
			...
			$url = plugins_url();
			global $news_magazine_spider_random_post_id;
	// Before widget //
...
if($news_magazine_spider_random_post_id==0){
...
<?php 
  ...
  ?>
<div  id="randarticle_<?php echo $news_magazine_spider_random_post_id ?>" >
<?php
global $news_magazine_spider_random_post_id;
echo "<script type='text/javascript'> Update(".$news_magazine_spider_random_post_id.",".$instance['Updating_Time'].",".$instance['Category'].",".$instance['quantity_of_posts'].",".$instance['Style_sra'].");  </script>";
	
$news_magazine_spider_random_post_id++;   
?>
</div>
<?php
	// After widget //
...
?>
<option value="<?php echo $categories[$i]->term_id?>" <?php selected($instance['Category'],$categories[$i]->term_id); ?>><?php echo $categories[$i]->name ?></option>
...
// End class news_magazine_spider_random_post

add_action('widgets_init', create_function('', 'return register_widget("news_magazine_spider_random_post");'));

add_action('wp_ajax_sprandom_post', 'news_magazine_spider_random_post_fornt_end');
add_action('wp_ajax_nopriv_sprandom_post', 'news_magazine_spider_random_post_fornt_end');

function news_magazine_spider_random_post_fornt_end(){
